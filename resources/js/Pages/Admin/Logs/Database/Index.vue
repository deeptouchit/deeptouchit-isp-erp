...
const props = defineProps({
    ...
    stats: {
        ...
        default: () => ({
            ...
            warn_count: 0,
            slow_count: 0,
            active_engine: 'MySQL Community Server',
            engine_health: { mysql: 'active', postgres: 'active', status: 'Healthy' },
        }),
    },
    filters: {
        type: Object,
        default: () => ({ source: 'mysql_error', lines: 100, search: '', severity: 'all' }),
    },
})
...
const downloadLog = () => {
    window.location.href = route('admin.logs.database.download', { source: currentSource.value })
}
...
<template>
    <Head title="Database Logs - DeepTouchHost" />
        ...
        <div class="w-full space-y-3.5">
            ...
            <PageHeader
                :breadcrumbs="[
                    ...
                    { label: 'Database Engine & Slow Query Logs' }
                ]"
            >
                <template #actions>
                    <button
            ...
            <InfoCardsGrid :cols="4">
                ...
                <InfoCard
                    title="Slow Query Events (>1s)"
                    :value="String(stats.slow_count || 0)"
                    badge="Performance"
                    badgeType="warning"
                    color="purple"
                    :icon="ClockIcon"
            ...
            <div class="flex items-center justify-between border-b border-slate-200 pb-2">
                <div class="flex flex-wrap items-center gap-2">
                    <button
                        v-for="src in ['mysql_error', 'mysql_slow', 'postgres_error', 'postgres_log']"
                        :key="src"
                        type="button"
            ...
            <DataTableFilter
                v-model="search"
                searchPlaceholder="Search database query, thread ID, or error message..."
                @search="applyFilters"
                @filter="applyFilters"
                @reset="resetFilters"
            >
                    ...
                    <table class="w-full text-center border-collapse text-xs whitespace-nowrap">
                        <thead>
                            <tr class="bg-slate-50/90 border-b border-slate-200 text-[10.5px] font-bold uppercase tracking-wider text-slate-600 whitespace-nowrap">
                                ...
                                <th class="py-2.5 px-3 border-r border-slate-200 text-left">Database Log Payload</th>
