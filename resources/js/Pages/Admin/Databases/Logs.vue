...
// Filters
const search = ref('')
const selectedEngine = ref(props.currentEngine || 'mysql')
...
const applyFilters = () => {
    router.get(route('admin.databases.logs'), {
...
// Filtered Engine Log entries
const filteredLogEntries = computed(() => {
    let list = props.engineLogs?.entries || []
...
// Clear Logs
const clearLogFile = () => {
    if (confirm(`Truncate and clear log file for ${selectedEngine.value.toUpperCase()}?`)) {
        useForm({ engine: selectedEngine.value }).post(route('admin.databases.logs.clear'), {
            preserveScroll: true,
            onSuccess: () => {
                feedbackMsg.value = `${selectedEngine.value.toUpperCase()} log file truncated.`
...
<template>
    <Head :title="`${selectedEngine.toUpperCase()} Logs & Diagnostics - DeepTouchHost`" />
        ...
        <div class="w-full space-y-3.5">
            ...
            <PageHeader
                :breadcrumbs="[
                    ...
                    { label: `${selectedEngine.toUpperCase()} Logs & Diagnostics` }
                ]"
            >
                <template #actions>
                    <Link
                    ...
                    <div class="flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-[3px] border border-slate-200 shadow-2xs text-xs">
                        <span class="font-bold text-slate-500">Engine:</span>
                        <select 
                            :value="selectedEngine"
                            @change="switchEngine($event.target.value)"
            ...
            <InfoCardsGrid :cols="4">
                ...
                <InfoCard
                    ...
                    :badge="stats.error_count > 0 ? 'Alert' : 'Clean'"
            ...
            <DataTableFilter
                v-model="search"
                searchPlaceholder="Search SQL logs, slow queries, or thread events..."
                @search="() => {}"
                @filter="applyFilters"
                @reset="resetFilters"
            >
                <FilterSelect
                    ...
                    :options="[
                        ...
                        { label: 'Errors & Critical Only', value: 'error' },
                        { label: 'Warnings & Slow Only', value: 'warning' },
                        { label: 'Notices & Notes', value: 'note' }
                    ]"
            ...
            <div class="bg-white rounded-[4px] border border-[#E2E8F0] shadow-2xs overflow-hidden">
                ...
                <div class="flex items-center border-b border-slate-200 bg-slate-50/70 px-3">
                    <button
                        ...
                        :class="activeTab === 'engine' ? 'border-blue-600 text-blue-600 font-bold bg-white' : 'border-transparent text-slate-600 hover:text-slate-900'"
                        class="py-2.5 px-3 text-xs border-b-2 transition cursor-pointer flex items-center gap-1.5"
                    >
                        <DocumentTextIcon class="w-3.5 h-3.5" />
                        <span>{{ selectedEngine.toUpperCase() }} Engine Logs ({{ filteredLogEntries.length }})</span>
                    </button>
                    ...
                    >
                        ...
                        <span>Database Mutation Audits ({{ auditLogs?.length || 0 }})</span>
                    </button>
                </div>

                <!-- Tab 1: Engine Logs Table -->
                <div v-if="activeTab === 'engine'" class="overflow-x-auto">
                    <table class="w-full text-center border-collapse text-xs whitespace-nowrap">
                        <thead>
                            <tr class="bg-slate-50/90 border-b border-slate-200 text-[10.5px] font-bold uppercase tracking-wider text-slate-600 whitespace-nowrap">
                                ...
                                <th class="py-2.5 px-3 border-r border-slate-200">Process / Thread</th>
                                <th class="py-2.5 px-3 text-left">Message & SQL Query Diagnostics</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 text-slate-700 font-medium whitespace-nowrap">
                            <tr v-for="(entry, index) in filteredLogEntries" :key="index" class="hover:bg-blue-50/30 transition">
                                <!-- # -->
                            ...
                            <tr v-if="!filteredLogEntries || filteredLogEntries.length === 0">
                                <td colspan="5" class="py-12 text-center text-slate-400">
                                    No database log entries found.
                                </td>
                            </tr>
                        </tbody>
                    </table>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700 font-medium whitespace-nowrap">
                            ...
                            <tr v-if="!auditLogs || auditLogs.length === 0">
                                <td colspan="7" class="py-12 text-center text-slate-400">
                                    No database mutation audits recorded.
                                </td>
                            </tr>
                        </tbody>
                    </table>
