...
@section('title', 'Support Tickets - ' . ($reseller->name ?? 'Sub-ISP Portal'))
...
@section('content')
<div class="space-y-3" x-data="resellerTicketsManager()" @scroll.window="activeMenu = null" @resize.window="activeMenu = null">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 bg-white px-4 py-2.5 rounded-xl border border-slate-200 shadow-xs">
        <div class="flex items-center gap-2.5">
            ...
            <h1 class="text-sm font-bold text-slate-800 tracking-tight">{{ __('Reseller Support Tickets') }}</h1>
        </div>
    ...
    <form method="GET" action="{{ route('reseller.tech.tickets') }}" class="bg-white p-2.5 rounded-xl border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2 flex-1 min-w-[240px]">
            <div class="relative flex-1">
                <i class="fas fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('Search Ticket Number, Subject...') }}" class="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:border-teal-500 shadow-2xs">
            </div>
            <select name="status" class="bg-slate-50 border border-slate-200 rounded-lg text-xs px-2 py-1.5 text-slate-700 focus:bg-white focus:border-teal-500">
        ...
        <div class="flex items-center gap-1.5">
            ...
            <a href="{{ route('reseller.tech.tickets') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs py-1.5 px-2.5 rounded-lg border border-slate-200/80 transition flex items-center justify-center gap-1 cursor-pointer">
                ...
                <tbody>
                    @forelse($tickets as $index => $ticket)
                        <tr>
                            ...
                            <td class="font-semibold text-slate-700">{{ $ticket->customer?->username ?? 'Subscriber' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-6 text-slate-400">
                                ...
                                <span>{{ __('No reseller support tickets found.') }}</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div x-show="showStatusModal" 
        ...
        <div class="bg-white rounded-xl border border-slate-200/90 shadow-2xl w-full max-w-md overflow-hidden" 
            ...
            <form :action="'/reseller/tech/tickets/' + selectedTicket?.id + '/status'" method="POST" class="p-4 space-y-3">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" placeholder="e.g. Field issue inspected and rectified." class="w-full bg-slate-50 border border-slate-200 rounded-lg text-xs p-2.5 focus:bg-white focus:border-amber-500 shadow-2xs"></textarea>
                </div>
...
@push('scripts')
<script>
function resellerTicketsManager() {
    return {
