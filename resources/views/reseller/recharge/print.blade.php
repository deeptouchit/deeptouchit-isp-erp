...
<head>
    ...
    <title>Wallet Recharge Statement - {{ $reseller->name ?? 'Partner' }}</title>
...
<body class="bg-slate-50 text-slate-800 text-xs antialiased font-sans p-4 sm:p-6 print:p-0 print:bg-white">
    ...
    <div class="max-w-4xl mx-auto mb-4 flex items-center justify-between no-print bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
        <div class="flex items-center gap-2">
            <span class="font-bold text-slate-800 text-sm">Wallet Recharge Statement &amp; History</span>
            <span class="text-xs text-slate-500 font-mono">{{ $month !== 'all' ? 'Month: ' . $month : 'All Records' }}</span>
        </div>
        <div class="flex items-center gap-2">
    ...
    <div class="max-w-4xl mx-auto bg-white border border-slate-200 rounded-xl p-6 print:border-none print:p-0 print:shadow-none space-y-5">
        ...
        <div class="flex items-start justify-between border-b border-slate-200 pb-4">
            <div>
                ...
                <p class="text-[11px] text-slate-500">Wallet Recharge Statement • {{ $tenant->company_name ?? $tenant->name ?? 'ISP Management System' }}</p>
            ...
            <div class="text-right">
                <div class="inline-block px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-mono font-bold text-xs border border-emerald-200">
                    APPROVED: @currency($totalApproved)
                </div>
                <div class="text-[11px] text-slate-500 font-mono mt-1.5">Total Topups: {{ count($recharges) }} Requests</div>
        ...
        <!-- 2. Recharges Table -->
        <div class="space-y-2">
            <h2 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Recharge &amp; Top-up Transactions</h2>

            <table class="w-full border-collapse text-[10.5px]">
                <thead>
                    <tr class="bg-slate-100 border-b border-slate-200 text-slate-700">
                        <th class="py-1.5 px-2 text-left font-semibold w-8">#</th>
                        <th class="py-1.5 px-2 text-left font-semibold w-32">Recharge #</th>
                        <th class="py-1.5 px-2 text-center font-semibold w-24">Method</th>
                        <th class="py-1.5 px-2 text-left font-semibold">Reference / TrxID / Bank</th>
                        <th class="py-1.5 px-2 text-right font-semibold w-28">Amount</th>
                        <th class="py-1.5 px-2 text-center font-semibold w-24">Status</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($recharges as $index => $r)
                        <tr>
                            ...
                            <td class="py-1.5 px-2 font-mono font-bold text-emerald-700">{{ $r->recharge_no }}</td>
                            <td class="py-1.5 px-2 text-center font-mono">{{ $r->payment_method_badge['label'] ?? $r->payment_method }}</td>
                            <td class="py-1.5 px-2 text-slate-700 font-mono text-[10px]">
                                @if($r->gateway_trx_id)
                                    <span>TrxID: {{ $r->gateway_trx_id }}</span>
                                @elseif($r->bank_name)
                                    <span>{{ $r->bank_name }} ({{ $r->bank_branch ?? 'Branch' }})</span>
                                @else
                                    <span>{{ $r->notes ?? 'Direct Topup' }}</span>
                                @endif
                            </td>
                            <td class="py-1.5 px-2 text-right font-mono font-bold text-slate-900">@currency($r->total_credited > 0 ? $r->total_credited : $r->amount)</td>
                            ...
                            <td class="py-1.5 px-2 text-center font-mono text-slate-600">{{ $r->created_at ? $r->created_at->format('Y-m-d') : '—' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-4 text-slate-400">No recharge records found.</td>
                        </tr>
                    @endforelse
                </tbody>
                <tfoot>
                    <tr class="bg-slate-50 border-t-2 border-slate-300 font-bold">
                        <td colspan="4" class="py-2 px-2 text-right text-slate-800">Total Approved Top-ups:</td>
                        <td class="py-2 px-2 text-right font-mono text-emerald-700">@currency($totalApproved)</td>
                        <td colspan="2"></td>
                    </tr>
                </tfoot>
            </table>
