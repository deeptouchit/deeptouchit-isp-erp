...
@section('title', 'New Customer - ' . ($reseller->name ?? 'Reseller Portal'))
...
<div class="max-w-4xl mx-auto space-y-4">
    ...
    <div class="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 shadow-xs">
        <div class="flex items-center gap-2.5">
            ...
            <h1 class="text-sm font-bold text-slate-800 tracking-tight">{{ __('New Customer') }}</h1>
        </div>
    ...
    {{-- Form Container --}}
    <form action="{{ route('reseller.customers.store') }}" method="POST" class="space-y-4">
        @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <input type="text" 
                           name="alt_phone" 
                           value="{{ old('alt_phone') }}" 
                           placeholder="{{ __('Alternative Phone') }}"
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('zone') }}" 
                           placeholder="e.g. Block-A, Road 5"
                           class="w-full px-3 py-1.5 rounded-lg text-xs bg-slate-50 border border-slate-200 text-slate-800 focus:bg-white focus:outline-none focus:border-cyan-500 shadow-2xs">
                ...
                <div>
                    ...
                    <select name="package_id" 
                        ...
                        @foreach($packages as $p)
                            <option value="{{ $p->id }}" data-price="{{ $p->price }}" {{ old('package_id') == $p->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="router_id" class="w-full px-2.5 py-1.5 rounded-lg text-xs bg-slate-50 border border-slate-200 text-slate-800 focus:bg-white focus:outline-none focus:border-cyan-500 shadow-2xs">
                        ...
                        @foreach($routers as $r)
                            <option value="{{ $r->id }}" {{ old('router_id') == $r->id ? 'selected' : '' }}>{{ $r->name }} ({{ $r->ip_address }})</option>
                        @endforeach
                    </select>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pkgSelect && billInput) {
        pkgSelect.addEventListener('change', function() {
            ...
            if (price && !billInput.value) {
                billInput.value = parseFloat(price).toFixed(2);
            }
        });
    }
});
