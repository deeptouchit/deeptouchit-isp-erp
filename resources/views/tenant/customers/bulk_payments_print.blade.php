...
<head>
    ...
    <title>Bulk Payments &amp; Due Statement - {{ $tenant->company_name ?? $tenant->name ?? 'ISP Management' }}</title>
...
<body class="bg-slate-50 text-slate-800 text-xs antialiased font-sans p-4 sm:p-6 print:p-0 print:bg-white">
    ...
    <div class="max-w-6xl mx-auto mb-4 flex items-center justify-between no-print bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
        <div class="flex items-center gap-2">
            ...
            <div>
                <span class="font-bold text-slate-800 text-sm block leading-tight">Bulk Bill &amp; Due Payments Statement</span>
                <span class="text-[11px] text-slate-500 font-mono">Scope: {{ $mode === 'reseller' ? ($reseller ? $reseller->name . ' (' . $reseller->code . ')' : 'Reseller Scope') : 'ISP Direct Retail' }} • Total: {{ count($subscribers) }} Subscribers</span>
            </div>
        </div>
        <div class="flex items-center gap-2">
    ...
    <div class="max-w-6xl mx-auto bg-white border border-slate-200 rounded-xl p-6 print:border-none print:p-0 print:shadow-none space-y-4">
        ...
        <div class="flex items-start justify-between border-b border-slate-200 pb-3">
            <div>
                ...
                <p class="text-xs text-slate-500 font-medium">Bulk Bill Collection &amp; Subscription Due Clearance Sheet</p>
                <div class="mt-1 text-[11px] text-slate-600 font-mono flex items-center gap-3">
                    <span>Generated: {{ now()->format('d M Y, h:i A') }}</span>
                    <span>•</span>
                    <span>Scope: <strong>{{ $mode === 'reseller' ? ($reseller ? $reseller->name . ' (' . $reseller->code . ')' : 'All Resellers') : 'ISP Direct Retail' }}</strong></span>
                </div>
            </div>
            <div class="text-right space-y-1">
                ...
                @if($mode === 'reseller' && $reseller)
                    <div class="text-[11px] text-slate-600 font-mono">
                        Reseller Margin: <strong>{{ (float)$commissionRate }}%</strong>
                    </div>
                @endif
            </div>
        </div>

        <!-- Summary KPI Cards Strip (4 Cards) -->
        <div class="grid grid-cols-4 gap-2">
            ...
            <div class="p-2 rounded-lg bg-indigo-50 border border-indigo-200">
                <span class="text-[9px] font-semibold text-indigo-800 uppercase block">Reseller Profit ({{ (float)$commissionRate }}%)</span>
            ...
            <div class="p-2 rounded-lg bg-emerald-50 border border-emerald-200">
                <span class="text-[9px] font-semibold text-emerald-800 uppercase block">Admin Share (Net Payable)</span>
        ...
        <table class="w-full border-collapse border border-slate-200 text-left text-[10.5px]">
            <thead>
                <tr class="bg-slate-100 text-slate-700 border-b border-slate-200 font-semibold">
                    ...
                    <th class="p-1.5 w-24 text-center">Collector Sign</th>
                </tr>
            </thead>
        ...
        <!-- Footer Signatures -->
        <div class="grid grid-cols-3 gap-6 pt-10 border-t border-slate-200">
            <div class="text-center">
                <div class="border-t border-slate-300 w-44 mx-auto mb-1"></div>
                <p class="text-[10.5px] font-semibold text-slate-700">Billing Officer / Prepared By</p>
                <p class="text-[9.5px] text-slate-400">{{ auth()->user()->name ?? 'System Operator' }}</p>
            </div>
            <div class="text-center">
                <div class="border-t border-slate-300 w-44 mx-auto mb-1"></div>
                <p class="text-[10.5px] font-semibold text-slate-700">{{ $mode === 'reseller' ? 'Reseller / Partner Sign' : 'Accounts Verified By' }}</p>
                <p class="text-[9.5px] text-slate-400">{{ $reseller ? $reseller->name : 'Finance Dept' }}</p>
            </div>
            <div class="text-center">
                <div class="border-t border-slate-300 w-44 mx-auto mb-1"></div>
                <p class="text-[10.5px] font-semibold text-slate-700">Authorized Seal &amp; Signature</p>
