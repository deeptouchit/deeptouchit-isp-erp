...
@section('title', 'Customer Action Bar - ' . ($tenant->company_name ?? $tenant->name))
...
@section('content')
<div class="space-y-3" 
     x-data="customerActionBarManager({
         customerId: {{ $customer->id }},
         customerStatus: '{{ $customer->status }}',
         pppoeUsername: '{{ addslashes($customer->username ?? '') }}',
         pppoePassword: '{{ addslashes($customer->password ?? '') }}',
         ipAddress: '{{ $customer->ip_address ?? '127.0.0.1' }}',
         packageId: '{{ $currentPackage?->id ?? ($customer->package_id ?? '') }}',
         packageName: '{{ addslashes($currentPackage ? ($currentPackage->package_name ?: $currentPackage->name) : ($customer->package_name ?: 'Custom Plan')) }}',
         packageCode: '{{ addslashes($currentPackage?->name ?? '') }}',
         packageSpeed: '{{ $currentPackage ? ($currentPackage->download_speed . '/' . $currentPackage->upload_speed . ' Mbps') : '' }}',
          monthlyBill: '{{ number_format((float)($customer->monthly_bill ?? ($currentPackage?->price ?? 0)), 2, '.', '') }}',
          expiryDate: '{{ $customer->expiry_date ? $customer->expiry_date->format('Y-m-d') : '' }}',
          expiryDateFormatted: '{{ $customer->expiry_date ? $customer->expiry_date->format('d M Y') : '' }}',
          billingType: '{{ strtolower($customer->billing_type ?: 'prepaid') }}',
          gracePeriodDays: {{ (int)($customer->grace_period_days ?? 0) }},
          autoCutEnabled: {{ $customer->auto_cut_enabled ? 'true' : 'false' }},
          dueAmount: '{{ number_format((float)($customer->due_amount ?? 0), 2, '.', '') }}',
          isReseller: {{ $customer->reseller_id ? 'true' : 'false' }},
         resellerId: '{{ $customer->reseller_id ?? '' }}',
         resellerName: '{{ addslashes($customer->reseller?->name ?? '') }}',
         resellerCode: '{{ addslashes($customer->reseller?->code ?? '') }}',
         ...
         isOnline: {{ ($customer->is_online ?? false) ? 'true' : 'false' }}
     })"
    ...
    <div class="bg-white px-3 py-1 rounded-lg border border-slate-200 shadow-2xs flex items-center justify-center relative overflow-visible z-10">
        <div class="card-header-scroll">
            <div class="btn-group">
                ...
                <!-- 5. Modify Monthly Bill -->
                <button type="button" 
                        @click="openModifyBillModal()" 
                        class="btn-hdr bg-pink cursor-pointer" 
                        data-title="{{ __('Modify Monthly Bill') }}">
                    <i class="fas fa-dollar-sign"></i>
                </button>

                <!-- 6. Change Billing Cycle -->
                <button type="button" 
                        @click="openBillingCycleModal()" 
                        class="btn-hdr bg-navy cursor-pointer" 
                        data-title="{{ __('Change Billing Cycle') }}">
                    <i class="fas fa-calendar-alt"></i>
                </button>

                <!-- 8. Update Due Balance -->
                <button type="button" 
                        @click="openDueModal()" 
                        class="btn-hdr bg-lime cursor-pointer" 
                        data-title="{{ __('Update Due Balance') }}">
                ...
                @if(!$authUser?->isCollector())
                <!-- 11. Transfer to Reseller -->
                <button type="button" 
                        @click="openTransferModal()" 
                        class="btn-hdr bg-indigo cursor-pointer" 
                        data-title="{{ __('Transfer to Reseller') }}">
                    <i class="fas fa-exchange-alt"></i>
                </button>
                @endif

                <!-- 12. Note for Customer -->
                <button type="button" 
                        @click="openNoteModal()" 
                    ...
                    <div class="p-3 flex-1 flex flex-col justify-between space-y-2.5">
                            ...
                            <div class="flex items-center gap-2.5">
                                ...
                                @else
                                    <div class="w-10 h-10 rounded-full bg-cyan-50 border border-cyan-200 text-cyan-600 flex flex-col items-center justify-center shadow-2xs flex-shrink-0">
                                        ...
                                        <span class="text-[6px] uppercase tracking-tighter text-slate-400 font-bold">NO IMAGE</span>
                                    </div>
                                @endif
                            ...
                            <div class="space-y-1.5 text-[11px]">
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Guardian</span>
                                    </div>
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Mobile</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                    @if($customer->phone)
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Reseller</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Area / Zone</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                    <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1">
                                        {{ $customer->zone ?: 'N/A' }}
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Sub Area</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                    <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1">
                        ...
                        <div class="space-y-1 mt-auto pt-1">
                            ...
                            <div class="space-y-0.5">
                                <span class="text-[10px] font-medium text-slate-500 block">Address:</span>
                                <p class="text-[10.5px] font-normal text-slate-700 leading-relaxed bg-slate-50/80 p-1.5 rounded border border-slate-200/70 truncate" title="{{ $customer->address ?: 'No address specified.' }}">
                    ...
                    <div class="p-3 flex-1 flex flex-col justify-between space-y-2.5">
                            ...
                            <div class="space-y-1.5 text-[11px]">
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Bandwidth</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Monthly Fee</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Billing Type</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                    <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 capitalize font-mono"
                                          x-text="(cycleData.billing_type || '{{ $customer->billing_type }}').toUpperCase()">
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Expire Date</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                    <span class="font-bold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono"
                                          x-text="formatDate(cycleData.expiry_date) || '{{ $customer->expiry_date?->format('d M Y') ?? 'N/A' }}'">
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Grace Period</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                    <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                                        <span x-text="cycleData.grace_period_days ?? '{{ $customer->grace_period_days ?? 0 }}'">{{ $customer->grace_period_days ?? 0 }}</span> Days
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1 mt-auto pt-1">
                            ...
                            <div class="flex items-center justify-between p-1.5 rounded bg-slate-50/80 border border-slate-200/70">
                                <span class="text-[10px] font-medium text-slate-600">Auto-Cut System:</span>
                    ...
                    <div class="p-3 flex-1 flex flex-col justify-between space-y-2.5">
                        <div class="space-y-2.5">
                            ...
                            <div class="flex items-center gap-2.5">
                                ...
                                <div class="min-w-0 flex-1">
                                    <h3 class="text-xs font-bold text-slate-900 tracking-tight leading-snug truncate" title="Network & Telemetry">
                                        Network & Telemetry
                                    </h3>
                            ...
                            <div class="space-y-1.5 text-[11px]">
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">PPPoE</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                    <span class="font-bold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Password</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">IP Address</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                                ...
                                <div class="flex items-center leading-tight whitespace-nowrap">
                                    <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                        ...
                                        <span class="text-[10px] font-medium text-slate-600">Uptime</span>
                                    </div>
                                    <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                        ...
                        <div class="space-y-1 mt-auto pt-1">
                            <div class="border-t border-slate-100"></div>
                            <div class="flex items-center justify-between p-1.5 rounded bg-slate-50/80 border border-slate-200/70">
                                <span class="text-[10px] font-medium text-slate-600">Router / NAS:</span>
            ...
            <div class="bg-white rounded-md border border-slate-200 shadow-2xs overflow-hidden border-t-2 border-t-teal-600">
                ...
                <div class="p-2.5 border-b border-slate-200 flex items-center justify-between gap-2 bg-slate-50/70">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="px-2 py-0.5 rounded text-[9.5px] font-mono bg-teal-100 text-teal-800 font-bold border border-teal-200">
                            Latest {{ $recentPayments->count() }} Invoices
                        </span>
                        <button type="button" 
                                @click="openPayModal()" 
                        ...
                        <table class="saas-table w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center w-20 no-sort">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="flex items-center justify-center gap-1">
                                                ...
                                                @endif
                                                <a href="{{ route('tenant.customers.payments.receipt', $payment->id) }}" 
                                                   target="_blank"
                ...
                @else
                    <div class="p-8 text-center space-y-2.5">
                        ...
                        <div class="space-y-0.5">
                            <h4 class="text-xs font-bold text-slate-700">No Billing Transactions Recorded</h4>
                            <p class="text-[11px] text-slate-500">Collect bill payment to generate official money receipts and record customer ledger transactions.</p>
                        </div>
                        <button type="button" 
                                @click="openPayModal()" 
                            ...
                            <span>Collect First Payment</span>
                        </button>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-md border border-slate-200 shadow-2xs overflow-hidden border-t-2 border-t-blue-600">
                        ...
                        <table class="saas-table w-full">
                            <thead>
                                <tr>
                                    <th class="w-8 text-center">#</th>
                                    <th>Framed IP</th>
                                    <th>Caller MAC (Device)</th>
                                    <th>NAS / Router IP</th>
                                    <th>Connected (Start)</th>
                                    <th>Disconnected (Stop)</th>
                                    <th>Duration</th>
                ...
                @else
                    <div class="p-8 text-center space-y-2">
                        ...
                        <h4 class="text-xs font-bold text-slate-700">No RADIUS Session Records Found</h4>
                        <p class="text-[11px] text-slate-500">Historical PPPoE login and accounting sessions for <code class="font-mono font-bold">{{ $customer->username }}</code> will appear here.</p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-md border border-slate-200 shadow-2xs overflow-hidden border-t-2 border-t-purple-600">
                ...
                <div class="p-2.5 border-b border-slate-200 flex items-center justify-between gap-2 bg-slate-50/70">
                    ...
                    <span class="px-2 py-0.5 rounded text-[9.5px] font-mono bg-purple-100 text-purple-800 font-bold border border-purple-200">
                        Latest {{ $activityLogs->count() }} Log Entries
                    </span>
                </div>
                ...
                @else
                    <div class="p-8 text-center space-y-2">
                        ...
                        <h4 class="text-xs font-bold text-slate-700">No Activity Logs Found</h4>
                        <p class="text-[11px] text-slate-500">System actions and billing transactions related to this subscriber will be recorded here.</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="col-span-12 lg:col-span-3 space-y-3.5">
            ...
            <div class="w-full bg-white rounded-md border border-slate-200 shadow-2xs overflow-hidden flex flex-col justify-between border-t-2 border-t-indigo-600">
                <div class="p-3 space-y-2.5">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-xs font-bold text-slate-900 tracking-tight leading-snug truncate" title="Traffic Graph">
                                Traffic Graph
                            </h3>
                        </div>
                    ...
                    <div class="relative bg-white rounded border border-slate-200 overflow-hidden shadow-2xs font-mono">
                        ...
                        <div class="relative w-full h-32 bg-white cursor-crosshair">
                            ...
                            <div x-show="!traffic.is_online" class="absolute inset-0 bg-slate-100/90 backdrop-blur-2xs flex flex-col items-center justify-center text-slate-500 gap-1">
                                ...
                                <span class="text-[9.5px] font-mono font-medium text-slate-600">Client is Offline</span>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-slate-200 bg-slate-50/70 text-[9.5px] font-mono divide-y divide-slate-100">
                            ...
                            <!-- Average Row -->
                            <div class="grid grid-cols-3 px-2 py-0.5 items-center hover:bg-white transition">
                                <span class="text-slate-500 font-medium">Average:</span>
                            ...
                            <!-- Max (Peak) Row -->
                            <div class="grid grid-cols-3 px-2 py-0.5 items-center hover:bg-white transition">
                                <span class="text-slate-500 font-medium">Peak (Max):</span>
                            ...
                            <div class="grid grid-cols-3 px-2 py-1 items-center bg-slate-100/50 font-semibold border-t border-slate-200">
                                <span class="text-slate-600">Total Volume:</span>
                ...
                <button type="button" 
                    ...
                    <span>Detailed Live Telemetry & Logs</span>
                </button>
            </div>
            ...
            <div class="w-full bg-white rounded-md border border-slate-200 shadow-2xs overflow-hidden flex flex-col justify-between border-t-2 border-t-purple-600">
                <div class="p-3 space-y-2.5">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="text-xs font-bold text-slate-900 tracking-tight leading-snug truncate" title="Device & Optical Details">
                                Device & Optical Details
                            </h3>
                    ...
                    <div class="space-y-1.5 text-[11px]">
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">Interface</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                            <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">ONU Name</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                            <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">ONU Type</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">ONU MAC</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                            <span class="font-bold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">Power</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">Distance</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">VLAN</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                            <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">Router MAC</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                            <span class="font-bold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono"
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">Router Brand</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                            <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                        ...
                        <div class="flex items-center leading-tight whitespace-nowrap">
                            <div class="w-[100px] flex items-center gap-1.5 text-slate-500 flex-shrink-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600">OLT</span>
                            </div>
                            <span class="text-slate-400 font-bold px-1 flex-shrink-0">:</span>
                            <span class="font-semibold text-slate-800 text-right flex-1 truncate text-[11px] pl-1 font-mono">
                    ...
                    <div class="space-y-0.5">
                        <span class="text-[10px] font-medium text-slate-500 block">Location:</span>
                        <p class="text-[10.5px] font-normal text-slate-700 leading-relaxed bg-slate-50/80 p-1.5 rounded border border-slate-200/70 truncate" 
                           title="{{ $customer->fiber_route_info ?: ($customer->zone ?: ($customer->address ?: 'No location specified.')) }}">
                            <i class="fas fa-map-marker-alt text-rose-500 text-[9px] mr-1"></i>
                            {{ $customer->fiber_route_info ?: ($customer->zone ?: ($customer->address ?: 'No location specified.')) }}
                        </p>
                    </div>
                    ...
                    <div class="space-y-1">
                        <div class="flex items-center justify-between text-[10px] font-medium text-slate-600">
                            <span class="flex items-center gap-1 font-semibold text-slate-700">
                                ...
                                <span>Visual Route & GPS Map</span>
                            </span>
                            @php
                ...
                <a href="{{ Route::has('tenant.network.onu') ? route('tenant.network.onu') : (Route::has('tenant.network.olt') ? route('tenant.network.olt') : '#') }}" 
                   class="w-full py-1.5 bg-purple-600 hover:bg-purple-700 text-white font-semibold text-[11px] flex items-center justify-center gap-1.5 transition shadow-2xs">
                    ...
                    <span>Diagnose Optical Line</span>
                </a>
            </div>
            ...
            <div class="w-full bg-white rounded-md border border-slate-200 shadow-2xs overflow-hidden flex flex-col justify-between border-t-2 border-t-amber-500">
                <div class="p-3 space-y-2.5">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-xs font-bold text-slate-900 tracking-tight leading-snug truncate" title="Customer Note & Remarks">
                                Note &amp; Remarks
                            </h3>
                        </div>
                        <button type="button" 
                    ...
                    <div class="p-2.5 rounded-lg bg-amber-50/40 border border-amber-200/60 text-slate-700 text-xs leading-relaxed min-h-[65px] whitespace-pre-line break-words">
                        ...
                        <template x-if="!noteForm.remarks || noteForm.remarks.trim() === ''">
                            <span class="text-slate-400 italic text-[11px] flex items-center gap-1">
                                ...
                                No internal notes or operational remarks recorded.
                            </span>
                        </template>
                    </div>
                </div>
                ...
                <button type="button" 
                    ...
                    <span>Manage Notes &amp; Remarks</span>
                </button>
            </div>
    ...
    @include('tenant.customers.components.modifybill')
    @include('tenant.customers.components.billingcycle')
    @include('tenant.customers.components.updatedue')
    @include('tenant.customers.components.paybill')
    @include('tenant.customers.components.transferreseller')
...
@push('scripts')
<script>
function customerActionBarManager(config) {
    return {
        ...
        noteForm: {
            remarks: '{{ addslashes($customer->remarks ?? '') }}'
        },
        payForm: {
            payment_mode: 'due',
            amount: '0.00',
        ...
        billForm: {
            monthly_bill: Number(config.monthlyBill || 0).toFixed(2),
            current_monthly_bill: Number(config.monthlyBill || 0).toFixed(2),
            reason: '',
            adjust_due: false
        },
        dueData: {
        ...
        cycleData: {
            ...
            currentExpiryDateFormatted: config.expiryDateFormatted || '',
            remainingDays: (() => {
                if (!config.expiryDate) return 0;
                const today = new Date(); today.setHours(0,0,0,0);
                const exp = new Date(config.expiryDate); exp.setHours(0,0,0,0);
        ...
        cycleForm: {
            billing_type: config.billingType || 'prepaid',
            expiry_date: config.expiryDate || '',
            grace_period_days: parseInt(config.gracePeriodDays) || 0,
            auto_cut_enabled: !!config.autoCutEnabled,
            reactivate_line: true
        },
        transferData: {
        ...
        async fetchUsageHistory() {
            ...
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/usage-history`, {
        ...
        openModifyBillModal() {
            this.closeAllModals();
            this.billForm.monthly_bill = Number(this.packageData.currentMonthlyBill || 0).toFixed(2);
            this.billForm.current_monthly_bill = Number(this.packageData.currentMonthlyBill || 0).toFixed(2);
            this.billForm.reason = '';
            this.billForm.adjust_due = false;
            this.billModal = true;
        },

        openBillingCycleModal() {
            this.closeAllModals();
            this.cycleForm.billing_type = this.cycleData.currentBillingType || 'prepaid';
            this.cycleForm.expiry_date = this.cycleData.currentExpiryDate || '';
            this.cycleForm.grace_period_days = this.cycleData.gracePeriodDays || 0;
            this.cycleForm.auto_cut_enabled = this.cycleData.autoCutEnabled;
            this.cycleForm.reactivate_line = true;
            this.cycleModal = true;
        },

        openDueModal() {
            this.closeAllModals();
            this.dueForm.mode = 'deduct';
            this.dueForm.amount = '';
            this.dueForm.remarks = '';
            this.dueModal = true;
        },
        ...
        async approvePayment(id, invoiceNo) {
            if (typeof Swal !== 'undefined') {
                const result = await Swal.fire({
                    title: 'Approve Payment & Activate Line?',
                    html: `<div class="text-xs text-slate-600">Are you sure you want to approve receipt <b>${invoiceNo}</b>?<br><span class="text-slate-400 mt-1 block">Subscriber line will be activated immediately on MikroTik and validity extended.</span></div>`,
                    icon: 'question',
                ...
                if (!confirm(`Are you sure you want to approve payment '${invoiceNo}' and activate this subscriber line?`)) {
                    return;
                }
            }

            try {
                const res = await fetch(`{{ url('/admin/finance/payments') }}/${id}/approve`, {
                    method: 'POST',
                    headers: {
        ...
        async fetchTrafficPoint() {
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/traffic`, {
                    headers: { 'Accept': 'application/json' }
                });
                const res = await response.json();
                if (res.success) {
        ...
        initGraph() {
            const canvas = document.getElementById('rxtxCanvas');
            if (!canvas) return;
        ...
        async submitPppoeUpdate() {
            ...
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/update-pppoe`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        async submitStatusUpdate() {
            ...
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/update-status`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        async submitPackageChange() {
            ...
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/update-package`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                ...
                if (res.success) {
                    ...
                    this.billForm.monthly_bill = res.monthly_bill;
                    this.billForm.current_monthly_bill = res.monthly_bill;
                    if (res.expiry_date) {
        ...
        async submitModifyBill() {
            if (!this.billForm.monthly_bill || Number(this.billForm.monthly_bill) < 0) {
                this.showToast('Please enter a valid monthly bill amount.', 'error');
                return;
            }
            this.isBillLoading = true;
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/update-monthly-bill`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                ...
                if (res.success) {
                    ...
                    this.billForm.current_monthly_bill = res.monthly_bill;
                    this.billForm.monthly_bill = res.monthly_bill;
                    this.showToast(res.message || 'Monthly bill updated successfully!');
                    this.billModal = false;
                } else {
                    this.showToast(res.message || 'Failed to update monthly bill.', 'error');
                }
            } catch (err) {
                this.showToast('Network error while updating monthly bill.', 'error');
            } finally {
        ...
        getCalculatedDays() {
            if (!this.cycleForm.expiry_date) return 0;
            const today = new Date(); today.setHours(0,0,0,0);
            const chosen = new Date(this.cycleForm.expiry_date); chosen.setHours(0,0,0,0);
            return Math.ceil((chosen - today) / (1000 * 60 * 60 * 24));
        },

        extendExpiryDays(days) {
            let base = new Date();
            if (this.cycleForm.expiry_date) {
                const cur = new Date(this.cycleForm.expiry_date);
                if (cur > base) {
                    base = cur;
                }
            }
            base.setDate(base.getDate() + days);
            this.cycleForm.expiry_date = base.toISOString().split('T')[0];
        },

        setToNextMonthFirst() {
            const now = new Date();
            const nextMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1);
            this.cycleForm.expiry_date = nextMonth.toISOString().split('T')[0];
        },

        async submitBillingCycle() {
            if (!this.cycleForm.expiry_date) {
                this.showToast('Please select an expiry date.', 'error');
                return;
            }
            this.isCycleLoading = true;
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/update-billing-cycle`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                    ...
                    body: JSON.stringify(this.cycleForm)
                });
                const res = await response.json();
                if (res.success) {
                    this.cycleData.currentBillingType = res.billing_type ? res.billing_type.toLowerCase() : this.cycleForm.billing_type;
                    this.cycleData.currentExpiryDate = res.expiry_raw || this.cycleForm.expiry_date;
                    this.cycleData.currentExpiryDateFormatted = res.expiry_date || '';
                    this.cycleData.remainingDays = res.remaining_days !== undefined ? res.remaining_days : this.getCalculatedDays();
                    this.cycleData.gracePeriodDays = res.grace_period_days !== undefined ? res.grace_period_days : this.cycleForm.grace_period_days;
                    this.cycleData.autoCutEnabled = res.auto_cut_enabled !== undefined ? res.auto_cut_enabled : this.cycleForm.auto_cut_enabled;
                    
                    this.packageData.currentExpiryDate = this.cycleData.currentExpiryDate;
                    this.packageData.remainingDays = this.cycleData.remainingDays;

                    this.showToast(res.message || 'Billing cycle updated successfully!');
                    this.cycleModal = false;
                } else {
                    this.showToast(res.message || 'Failed to update billing cycle.', 'error');
                }
            } catch (err) {
                this.showToast('Network error while updating billing cycle.', 'error');
            } finally {
        ...
        calculateNewDue() {
            const cur = parseFloat(this.dueData.currentDue) || 0;
            const amt = parseFloat(this.dueForm.amount) || 0;
            if (this.dueForm.mode === 'clear') return 0;
            if (this.dueForm.mode === 'set') return Math.max(0, amt);
            if (this.dueForm.mode === 'add') return cur + amt;
            if (this.dueForm.mode === 'deduct') return Math.max(0, cur - amt);
            return cur;
        },

        async submitDueBalance() {
            if (this.dueForm.mode !== 'clear' && (this.dueForm.amount === '' || Number(this.dueForm.amount) < 0)) {
                this.showToast('Please enter a valid amount.', 'error');
                return;
            }
            this.isDueLoading = true;
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/update-due-balance`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                    ...
                    body: JSON.stringify(this.dueForm)
                });
                const res = await response.json();
                if (res.success) {
                    this.dueData.currentDue = parseFloat(res.due_amount_raw !== undefined ? res.due_amount_raw : (res.due_amount || 0));
                    this.showToast(res.message || 'Due balance updated successfully!');
                    this.dueModal = false;
                } else {
                    this.showToast(res.message || 'Failed to update due balance.', 'error');
                }
            } catch (err) {
                this.showToast('Network error while updating due balance.', 'error');
            } finally {
                this.isDueLoading = false;
            }
        },
        ...
        async submitBillPayment() {
            ...
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/pay-bill`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                ...
                if (res.success) {
                    ...
                    // Open Money Receipt in a clean popup if invoice exists
                    if (res.payment && res.payment.id) {
                        const receiptUrl = `{{ url('admin/customers/payments') }}/${res.payment.id}/receipt`;
                        window.open(receiptUrl, '_blank', 'width=750,height=850,scrollbars=yes');
                    }
        ...
        async submitTransfer() {
            if (this.transferForm.target_type === 'reseller' && !this.transferForm.reseller_id) {
                this.showToast('Please select a target reseller partner.', 'error');
                return;
            }
            this.isTransferLoading = true;
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/transfer-reseller`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                ...
                const res = await response.json();
                if (res.success) {
                    this.transferData.currentResellerId = res.reseller_id;
                    this.transferData.currentResellerName = res.reseller_name;
                    this.transferData.currentResellerCode = res.reseller_code;
                    this.packageData.isReseller = !!res.reseller_id;
                    this.packageData.resellerId = res.reseller_id || '';
                    this.packageData.resellerName = res.reseller_name || '';
                    this.packageData.resellerCode = res.reseller_code || '';

                    this.showToast(res.message || 'Subscriber transferred successfully!');
                    this.transferModal = false;
                } else {
                    this.showToast(res.message || 'Failed to transfer subscriber.', 'error');
                }
            } catch (err) {
                this.showToast('Network error while transferring subscriber.', 'error');
            } finally {
        ...
        async submitCustomerNote() {
            ...
            try {
                const response = await fetch(`{{ url('admin/customers') }}/${this.customerId}/update-note`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
