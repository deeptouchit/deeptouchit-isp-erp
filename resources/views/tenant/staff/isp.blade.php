...
@section('title', 'ISP Operations Staff - ' . ($tenant->company_name ?? $tenant->name))
...
@section('content')
<div class="space-y-3" x-data="ispStaffManager()" @scroll.window="activeMenu = null" @resize.window="activeMenu = null">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-sm font-bold text-slate-900 tracking-tight">ISP Operations Staff</h1>
        </div>

        <div class="flex items-center gap-2">
            <!-- Onboard ISP Staff Button -->
            <button type="button" 
                    @click="openAddModal()"
                    class="px-3.5 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs shadow-xs transition inline-flex items-center gap-1.5 cursor-pointer">
                <i class="fas fa-user-plus text-[11px]"></i>
                <span>+ Onboard ISP Staff</span>
            </button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
        ...
        <div class="px-2.5 py-1.5 rounded-lg bg-white border border-slate-200 shadow-xs flex items-center justify-between">
            <div class="min-w-0">
                ...
                <span class="text-[13px] font-bold text-purple-600 font-mono leading-tight block">{{ number_format($totalIspStaff) }}</span>
            </div>
        ...
        <!-- Metric 3: NOC Engineers -->
        <div class="px-2.5 py-1.5 rounded-lg bg-white border border-slate-200 shadow-xs flex items-center justify-between">
            <div class="min-w-0">
                <span class="text-[9px] font-medium text-indigo-700 uppercase tracking-wider block truncate">NOC Engineers</span>
                <span class="text-[13px] font-bold text-indigo-600 font-mono leading-tight block">{{ number_format($nocCount) }}</span>
            </div>
        ...
        <!-- Metric 4: Field Technicians -->
        <div class="px-2.5 py-1.5 rounded-lg bg-white border border-slate-200 shadow-xs flex items-center justify-between">
            <div class="min-w-0">
                <span class="text-[9px] font-medium text-emerald-700 uppercase tracking-wider block truncate">Field Techs</span>
                <span class="text-[13px] font-bold text-emerald-600 font-mono leading-tight block">{{ number_format($fieldTechCount) }}</span>
            </div>
    ...
    <div class="bg-white p-3 rounded-xl border border-slate-200 shadow-xs">
        <form method="GET" action="{{ route('tenant.staff.isp') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-2.5 items-center">
            ...
            <div>
                <select name="department" 
                    ...
                    <option value="management" {{ request('department') === 'management' ? 'selected' : '' }}>Operations &amp; Admin</option>
                    <option value="noc" {{ request('department') === 'noc' ? 'selected' : '' }}>NOC / Server Eng</option>
                    <option value="field" {{ request('department') === 'field' ? 'selected' : '' }}>Field &amp; Fiber Tech</option>
                    <option value="billing" {{ request('department') === 'billing' ? 'selected' : '' }}>Retail Billing Dept</option>
                </select>
            </div>
                    ...
                    @forelse($employees as $index => $emp)
                        ...
                        <tr>
                            ...
                            <td class="text-slate-700">
                                {{ $emp->designation ?: 'ISP Operations' }}
                            </td>
                            ...
                            <!-- Shift -->
                            <td class="text-slate-600 text-[10.5px]">
                        ...
                        <tr>
                            <td colspan="9" class="text-center py-8 text-slate-400">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    <i class="fas fa-building-user text-2xl text-slate-300"></i>
                                    <span class="text-xs font-medium">No ISP HQ operations staff found matching your filters.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div x-show="activeMenu !== null" 
        ...
        <div class="py-1">
            <button type="button" 
                ...
                <i class="fas fa-id-card text-purple-500 w-4 text-center text-[11px]"></i>
        ...
        <div @click.outside="showAddModal = false" 
            ...
            <div class="bg-slate-50/80 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-xs font-semibold text-slate-800">Onboard ISP Core Staff</h3>
                        <p class="text-[10.5px] text-slate-500 font-normal">Add HQ manager, NOC engineer, field tech or collector</p>
                    </div>
                </div>
            ...
            <form @submit.prevent="submitAddEmployee()">
                <div class="p-4 space-y-3.5 text-xs max-h-[75vh] overflow-y-auto">
                    ...
                    <!-- Role Selector -->
                    <div>
                        <label class="block text-[10.5px] font-bold text-slate-700 uppercase tracking-wider mb-1">ISP Operational Role <span class="text-rose-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        <div>
                            ...
                            <select x-model="addForm.shift" class="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-800 focus:bg-white focus:border-purple-500 focus:outline-hidden transition">
        ...
        <div @click.outside="showEditModal = false" 
            ...
            <div class="bg-slate-50/80 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-xs font-semibold text-slate-800">Edit Staff Profile</h3>
                        <p class="text-[10.5px] text-slate-500 font-normal">Update contact details, role permissions &amp; assignment</p>
                    </div>
                </div>
        ...
        <div @click.outside="showViewModal = false" 
            ...
            <div class="bg-slate-50/80 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-xs font-semibold text-slate-800">Staff Full Profile</h3>
                        <p class="text-[10.5px] text-slate-500 font-normal">Detailed operational info &amp; department assignment</p>
                    </div>
                </div>
            ...
            <div class="p-4 space-y-3.5 text-xs text-slate-800 max-h-[75vh] overflow-y-auto">
                ...
                <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center text-sm font-bold border border-purple-200 shadow-2xs">
                        ...
                        <div>
                            ...
                            <div class="text-[10.5px] text-purple-700 font-medium" x-text="viewData?.designation || 'ISP Staff Member'"></div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div class="p-2 bg-slate-50 rounded-lg border border-slate-200/80">
                        ...
                        <span class="font-bold text-purple-700 text-[11px] mt-0.5 block truncate">ISP Headquarters</span>
                    </div>
                    <div class="p-2 bg-slate-50 rounded-lg border border-slate-200/80">
                        <span class="text-[9.5px] text-slate-500 uppercase block font-medium">Role Badge</span>
                        <span class="font-bold text-slate-800 text-[11px] mt-0.5 block truncate" x-text="viewData?.role_badge?.label || viewData?.role"></span>
                    </div>
                    <div class="p-2 bg-slate-50 rounded-lg border border-slate-200/80">
    ...
    return {
        ...
        async submitEditEmployee() {
            ...
            try {
                ...
                this.showToast('Network error while updating staff.', 'error');
            } finally {
                this.isSubmitting = false;
            }
        },
        ...
        async viewEmployee(id) {
            try {
                ...
                this.showToast('Failed to load staff details.', 'error');
            }
        },
